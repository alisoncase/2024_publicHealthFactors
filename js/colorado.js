...
// that displays health outcomes and health risk behaviors for each county in Colorado.
...
(function(){
    ...
    var initialFirstVariable = attrArray[2]; // cancer
    var initialSecondVariable = attrArray[12]; // binge_drinking
    ...
    function setMap(){
      ...
      var coloradoMap = d3.select("#coloradoMap") 
        .append("svg")
        .attr("class", "coloradoMap")
        .attr("d", path) 
        .attr("width", width)
        .attr("height", height);
    
      // Set main map projection to Albers and position it on Colorado
      ...
      var promises = [];    
        ...
        function callback(data) {
            ...
            // Filter counties and states for Colorado only
            var coloradoCounties = topojson.feature(counties, counties.objects.countiesNew2).features.filter(function(d) {
                return d.properties.REGION === "CO";
            });
            ...
            setEnumerationUnits(coloradoCounties, coloradoMap, path, colorScale, initialFirstVariable, initialSecondVariable);
        };
    ...
    function setEnumerationUnits(coloradoCounties, coloradoMap, path, colorScale, firstVariable, secondVariable){
      //add counties to map
      var enumerationUnits = coloradoMap.selectAll(".counties")
